...
lower_liab = 0
upper_liab = df_clean['Liabilities / Assets'].quantile(0.99)
before, after = process_indicator(
    df=df_clean,
...
lower_roa = df_clean['ROA'].quantile(0.01)
upper_roa = df_clean['ROA'].quantile(0.99)
before, after = process_indicator(
    df=df_clean,
    indicator_name='ROA',
...
lower_nm = df_clean['Net Margin'].quantile(0.01)
upper_nm = df_clean['Net Margin'].quantile(0.99)
before, after = process_indicator(
    df=df_clean,
...
lower_ebit = df_clean['EBIT / Assets'].quantile(0.01)
upper_ebit = df_clean['EBIT / Assets'].quantile(0.99)
before, after = process_indicator(
    df=df_clean,
...
with open('README_data_processing.md', 'w', encoding='utf-8') as f:
    ...
    # Stage 4a
    f.write('## Stage 4. Outlier treatment by columns\n\n')
    ...
    # Stage 4b
    f.write('## Stage 4. Balance sheet normalization\n\n')
    f.write('After individual outlier treatment, the accounting identity `Equity/Assets + Liabilities/Assets = 1` was violated.\n\n')
    ...
    f.write('**Solution:** Both ratios were divided by their sum.\n\n')
    ...
    f.write('For all added years, financial indicators were filled using the last available value from previous years.\n\n')

    missing_after = df_expanded[cols_to_fill].isnull().sum().sum()
    f.write(f'**Result:** {missing_after} missing values remaining in financial indicators.\n\n')

    f.write('**Note:** Default flags were not forward-filled. Only actual reporting years contain default information.\n\n')
    f.write('---\n\n')
